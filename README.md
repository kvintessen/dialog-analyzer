...
- В приложении пока нет UI/API для создания сообщений (только сидер) —
  `MessageObserver` и очередь готовы и покрыты тестами, но реального входа
  для «живых» сообщений (например, вебхука от чат-интеграции) ещё нет.
